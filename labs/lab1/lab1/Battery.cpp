...
Battery::Battery() {
	...
	this->lifeTimeMinutes = status.BatteryLifeTime/60%60;
	this->lifeTimeHours = status.BatteryLifeTime / 60 / 60;

	this->fullLifeTime = status.BatteryFullLifeTime / 60 / 60;
...
string Battery::getStateCharge(SYSTEM_POWER_STATUS status) {
	...
	//case 0:
	//	return "not charging"; //Значение равно нулю, если аккумулятор не заряжается и емкость аккумулятора находится в диапазоне от низкого до высокого.
	case 9:
		return "charging | high ( > 66% )";
	case 10:
		return "charging | low ( < 33% )";
	case 12:
		return "charging | critical ( < 5% )";
...
ostream& operator<<(ostream& out, Battery battery)
{
	out << "BATTERY" << endl
		...
		<< "  Life time: " << battery.lifeTimeHours << " h " <<battery.lifeTimeMinutes<< "m" << endl
		<< "  Full life time: " << battery.fullLifeTime << "h" << endl
		<< "  Chemistry: ";
